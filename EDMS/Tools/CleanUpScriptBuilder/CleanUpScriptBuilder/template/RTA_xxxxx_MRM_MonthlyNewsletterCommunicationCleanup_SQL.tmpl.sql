...
SELECT	@a13_a12_code			=	'Mass Communication',
		...
		@a13_delete_flag		=	0,
		@a13_a16_code			=	'Monthly Communication',
		@a13_communication_code =	'SMC_20130828'
-----------------------------------------------------
--Create Temp Table for List of ind_cst_keys provided by the business
SELECT	ind_cst_key
INTO	#TMP_Individuals
FROM	co_individual 
join co_customer (NOLOCK) on ind_cst_key = cst_key
WHERE	cst_recno IN (xxID_LISTxx)
-----------------------------------------------------
BEGIN TRANSACTION 
