...
function($, _, Backbone, SALT) {
  ...
  var wt = {
    ...
    trigger: function(eventName, object, force) {
      ...
      if (!force && preloadCheckPending) {

        //log whatif event
        if(eventName.indexOf('repaymentOptionSelect') > 0 || eventName.indexOf('compare') > 0 || eventName.indexOf('showBalanceGraph') > 0 || eventName.indexOf( 'updateGraph') > 0)
        {
        ...
        //exit event
        else if(eventName.indexOf('saveAndExit' ) > 0)
        {
          SALT.global.WT.publish(eventName, {
            lesson: additionalDetails,
            actionName: 'save',
  ...
  // Lesson 3 Analytics Subscriptions
  ...
  var app = {
    ...
    root: "/own-your-student-loans/", //QC-6289
